...
class Technologies:
    ...
    relevantColumns = ['LanguageHaveWorkedWith', 'DatabaseHaveWorkedWith', 'PlatformHaveWorkedWith',
                       'WebframeHaveWorkedWith', 'ToolsTechHaveWorkedWith']
    interested_professions = ['Full-Stack Developers', 'Embedded Developers', 'Backend Developer',
                              'Frontend developers', 'Data Engineer', 'Data Scientist', 'DevOps specialist',
                              'QA developers']
    ...
    def get_df_dict(self, df, final_dict, input_obj, k):
        ...
        for prof in self.interested_professions:
            final_dict[prof]['languages'] = input_obj.getkMostPopularTechnologies(input_obj.languageDf, prof,
                                                                                  ['ResponseId', 'abs_comp_k',
                                                                                   'DevType', 'curr_symbol'], k)
            final_dict[prof]['database'] = input_obj.getkMostPopularTechnologies(input_obj.databaseDf, prof,
                                                                                 ['ResponseId', 'abs_comp_k', 'DevType',
                                                                                  'curr_symbol'], k)
            final_dict[prof]['platform'] = input_obj.getkMostPopularTechnologies(input_obj.platformDf, prof,
                                                                                 ['ResponseId', 'abs_comp_k', 'DevType',
                                                                                  'curr_symbol'], k)
            final_dict[prof]['webFramework'] = input_obj.getkMostPopularTechnologies(input_obj.webFrameWorkDf, prof,
                                                                                     ['ResponseId', 'abs_comp_k',
                                                                                      'DevType', 'curr_symbol'], k)
            final_dict[prof]['tools'] = input_obj.getkMostPopularTechnologies(input_obj.toolsDf, prof,
                                                                              ['ResponseId', 'abs_comp_k', 'DevType',
                                                                               'curr_symbol'], k)
    ...
    def updateProf(self, df):
        ...
        df['DevType'] = np.where(df['DevType'] == 'Data scientist or machine learning specialist', 'Data Scientist',
                                 df['DevType'])
        df['DevType'] = np.where(df['DevType'] == 'Developer, back-end', 'Backend Developer', df['DevType'])
        df['DevType'] = np.where(df['DevType'] == 'Developer, embedded applications or devices', 'Embedded Developers',
                                 df['DevType'])
    ...
    def createDfs(self, df, column):
        ...
        if column == 'LanguageHaveWorkedWith':
            ...
            self.languageDf.dropna(axis=0, how='any', inplace=True)
        elif column == 'DatabaseHaveWorkedWith':
            self.databaseDf = bool_df
            self.databaseDf.dropna(axis=0, how='any', inplace=True)
        elif column == 'PlatformHaveWorkedWith':
            self.platformDf = bool_df
            self.platformDf.dropna(axis=0, how='any', inplace=True)
        elif column == 'WebframeHaveWorkedWith':
            self.webFrameWorkDf = bool_df
            self.webFrameWorkDf.dropna(axis=0, how='any', inplace=True)
        elif column == 'ToolsTechHaveWorkedWith':
            self.toolsDf = bool_df
            self.toolsDf.dropna(axis=0, how='any', inplace=True)
        else:
...
def plotly_bar(input_dict: dict, rel_cols: list, dict_key: str, x_axis: str, y_axis: str, title: str, angle=-45):
    ...
    Helper function to return plotly figure for bar graph
        ...
        :param rel_cols: input columns
        :type rel_cols: list
        :param dict_key: input key
        :type dict_key: str
    ...
    fig = px.bar(ef, x="index", y=['Full-Stack Developers', 'Backend Developer', 'Data Engineer', 'Data Scientist'],
                 labels={"index": x_axis, "variable": "Legend"}, title=title, barmode="group",
                 color_discrete_sequence=['rgba(244, 128, 36, 1)', 'rgba(34, 36, 38, 1)',
                                          'rgba(188, 187, 187, 1)', 'rgba(244, 36, 48, 1)'])
    fig.update_layout({
        'paper_bgcolor':'rgba(255, 203, 148, 1)',
