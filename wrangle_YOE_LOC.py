...
def wrangleYearsExperience(df):
    """
    Grabs relevant columns to experience and salary, and removes data points without insightful responses (N/As)
    :param df: [pd.DataFrame] Input Dataframe
    :return: A dataframe with year of experience
    """
    assert isinstance(df, pd.DataFrame), "Input has to be a pandas dataframe"
    assert all(x in df.columns.values for x in
               ["ResponseId", "YearsCodePro", "ConvertedCompYearly", "Currency", 'CompTotal', 'CompFreq'])
...
def wrangleLocation(df):
    """
    Grabs relevant columns to location and salary, and removes data points without insightful responses (N/As)
    :param df: [pd.DataFrame] Input Dataframe
    :return: A dataframe with US states
    """
    assert isinstance(df, pd.DataFrame), "Input has to be a pandas dataframe"
    assert all(x in df.columns.values for x in
               ["ResponseId", "US_State", "ConvertedCompYearly", "Currency", 'CompTotal', 'CompFreq'])
